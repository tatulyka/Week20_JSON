 ...
 { "name": "Дэдпул",
    ...
    "img": "./images/daeadpool.png"
 }
 ]`;

(function()  {
   const renderRate = (name, value) => {
      let options = '';
      for (let i = 1; i <= 10; i++) {
         const selected = value === i.toString() ? "selected" : "";
         options += `<option value="${i}" ${selected}>${i}</option>`;
      }
      return `
      <span class="rates__choise">
          <select name="${name}">
            ${options}
          </select>
      </span>
  ...
  const renderHero = (hero, ratingValue) => {
   return `
     <div class="basic__hero persone">
      <div class="persone__main">
         <div class="persone__image" ><img src="${hero.img}" class="hero__img">
         </div>
         <div class="persone__names">
            <div>Альтер Эго: ${hero.alter}</div>
            <h3>Имя: ${hero.name}</h3>
            <div>Род занятий: ${hero.occupation}</div>
            <div>Друзья: ${hero.friends}</div>
            <div>Враги: ${hero.forces}</div>
         </div>   
      </div>
      <div class="persone__container">
            Описание: ${hero.description}
         </div>
         ${renderRate(hero.name, ratingValue)}
     </div>
   `;
}
 const heroes = JSON.parse(json);
 const container = document.querySelector('.basic__heroestable');
 let content = '';
 heroes.forEach(hero => {
   const rating = localStorage.getItem(hero.name) || '1';
   content += renderHero(hero, rating);
 });
 ...
 document.querySelectorAll('select').forEach((select) => {
   select.addEventListener("change", handleChangeRating);
 });
}())
